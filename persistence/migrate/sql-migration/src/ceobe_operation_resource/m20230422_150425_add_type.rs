...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20230422_150425_ceobe_operation_resource_add_type"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        al.table(CeobeOperationResource::Table).add_column(
            &mut ColumnDef::new_with_type(
                CeobeOperationResource::CountdownType,
                CountdownType::column_type(),
            ),
        );
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        let mut al = sea_query::Table::alter();
        al.table(CeobeOperationResource::Table)
            .drop_column(CeobeOperationResource::CountdownType);
        manager.alter_table(al).await?;
