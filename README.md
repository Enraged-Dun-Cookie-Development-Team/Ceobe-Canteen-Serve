...
- 数据库异常（SeaOrm）
  ...
  | `D`  |  0009  |   500    | 数据库连接池耗尽                                |
  | `D`  |  000A  |   500    | 数据库尝试类型转换失败                          |
  | `D`  |  000B  |   500    | 数据库对应类型无法转换为`u64`                   |
  | `D`  |  000C  |   500    | 数据库无法获取 `last_insert_id`                 |
  | `D`  |  000D  |   500    | 数据库更新时未指定`primary key`                 |
  | `D`  | 000DE  |   500    | `ActiveModel` 内容不全是 `Set` 或者 `Unchanged` |
