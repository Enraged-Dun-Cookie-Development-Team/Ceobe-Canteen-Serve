...
impl PrepareDecorator for TimeoutDecorator {
    ...
    fn decorator<Fut, T>(&self, src: &'static str, in_fut: Fut) -> Self::OutFut<Fut, T> where Fut: Future<Output=Result<T, PrepareError>> + 'static, T: 'static {
        let duration = self.0;
        Box::pin(timeout(duration,in_fut)
            .map_err(move|_err|PrepareError::new(src,Box::new(Error(duration))))
