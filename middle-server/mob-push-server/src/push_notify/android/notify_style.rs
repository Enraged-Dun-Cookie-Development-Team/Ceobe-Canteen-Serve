...
impl NotifyStyle {
    /// 创建一个 **默认样式**  的推送消息格式
    pub fn new_default() -> Self { Self::Default }
...
impl NotifySerialize for NotifyStyle {
    ...
    fn serialize<S: serde::Serializer>(
        ...
        match self {
            ...
            NotifyStyle::Banner(info_vec) => {
                serialize_struct.serialize_field("content", info_vec)
            }
            NotifyStyle::Custom(inner) => {
                serialize_struct.serialize_field("customStyle", inner)
            }
        }
    }
}
