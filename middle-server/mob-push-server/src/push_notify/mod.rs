...
pub trait NotifySerialize {
    ...
    fn serialize<S: Serializer>(
        &self, struct_serialize: &mut <S as Serializer>::SerializeStruct,
...
impl<T: NotifySerialize> NotifySerialize for Option<T> {
    ...
    fn serialize<S: Serializer>(
        ...
        match self {
            Some(inner) => {
                NotifySerialize::serialize::<S>(inner, struct_serialize)
            }
            None => Ok(()),
        }
    }
...
pub trait SerializeInformation: NotifySerialize + Sized {
    ...
    fn into_notify(self) -> Notify<Self> { Notify(self) }
}
...
impl<N> Notify<N> {
    pub fn new(notify: N) -> Self { Self(notify) }
}
