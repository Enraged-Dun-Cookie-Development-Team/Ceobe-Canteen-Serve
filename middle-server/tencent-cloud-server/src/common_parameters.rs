...
use crate::{cloud_manager::CloudManager, config::TencentConfigTrait, error::TcCloudError};
...
impl CloudManager {
    ...
    fn sign<T: Serialize, P: Serialize>(&self, common_params: CommonParameter, request: RequestContent<T, P>) -> Result<String, TcCloudError> {
        ...
        let canonical_headers = format!("content-type:{}\nhost:{}\nx-tc-action:{}\n", request.content_type, host, common_params.action.to_lowercase());
        ...
        let canonical_request = format!("{}\n{}\n{}\n{}\n{}\n{}", request.method.to_string(), canonical_uri, canonical_query, canonical_headers, signed_headers, hashed_request_payload);

        let datetime = DateTime::from_timestamp(common_params.timestamp, 0).unwrap();
        let date = datetime.format("%Y-%m-%d").to_string();
        let credential_scope = format!("{}/{}/tc3_request", date, common_params.service);
        ...
        let secret_date = hmacsha256(&date, &format!("TC3{}", self.id.expose_secret()))?;
