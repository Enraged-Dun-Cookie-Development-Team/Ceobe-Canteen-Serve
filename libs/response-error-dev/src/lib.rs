...
pub fn encode_response_error(config: impl AsRef<Path>) {
    ...
    let cfg =
        std::fs::read_to_string(config).expect("Error Config File Not Exist");
    let err_cfg =
        toml::from_str::<ErrorCfg>(&cfg).expect("Invalid Config File Format");
    ...
    for error_kind_group in err_group {
        out.write_all(error_kind_group.to_string().as_bytes())
