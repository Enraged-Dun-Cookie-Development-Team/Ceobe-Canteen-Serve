...
use super::{
    FetchViewValue, GetOptionViewValue, OptionViewField, _private::SealTrait,
};
...
impl<T> GetOptionViewValue<T> for ValueField<T> {
    fn get_option(self) -> Option<T> { Some(self.0) }
}
