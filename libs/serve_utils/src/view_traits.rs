...
/// 用于view中的可选field
/// 
/// 由于view可能存面向不同端有细微差别，如果由于少量的字段区分重复整个view定义会带来大量代码冗余。
/// 因此，提出了该trait
/// 
/// ## Example
/// 如果有某个view结构体，
...
/// ```
/// 
/// - 当在后台接口中使用时，其为 `FooView<ValueField<i32>>`, 即需要提供id的view。可能的序列化结果为
/// ```json
/// {
///     "id": 114514,
///     "foo": "hello"
/// }
/// ```
/// - 当在前台接口中使用时，其为 `FooView<SkipField>`, 即不携带id的view。可能的序列化结果为
/// ```json
/// {
///     "foo": "hello"
/// }
/// ```
/// - 当需要根据是否提供ID执行不同操作实现，其为 `FooView<OptionValueField<i32>>`, 即携带Option的ID的view
