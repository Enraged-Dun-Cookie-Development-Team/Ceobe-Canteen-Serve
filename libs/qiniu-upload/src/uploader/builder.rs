...
impl UploaderBuilder {
    pub fn new(secret: &impl SecretConfig) -> Self {
        Self {
            credential: Credential::new(secret.access_key(), secret.secret_key()),
            managers: HashMap::with_hasher(ahash::RandomState::new()),
        }
    }

    pub fn add_bucket(mut self, name: &(impl AsRef<str> + ?Sized)) -> Result<Self, error::Error> {
        if !self.managers.contains_key(name.as_ref()) {
            let manage = UploadManager::builder(UploadTokenSigner::new_credential_provider(
