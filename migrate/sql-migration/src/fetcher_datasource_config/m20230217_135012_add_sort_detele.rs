...
impl MigrationTrait for Migration {
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        let mut al = sea_query::Table::alter();
        al.table(FetcherDatasourceConfig::Table).drop_column(
            FetcherDatasourceConfig::DeleteAt
        );
        let mut delete = sea_query::DeleteStatement::new();
        delete.from_table(FetcherDatasourceConfig::Table).and_where(Expr::col(FetcherDatasourceConfig::DeleteAt).ne(Value::ChronoDateTime(
            Box::new(get_zero_data_time()).into(),
        )));
