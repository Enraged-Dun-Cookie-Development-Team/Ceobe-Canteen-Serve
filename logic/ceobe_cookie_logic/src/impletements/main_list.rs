...
use db_ops_prelude::{mongo_connection::MongoDatabaseOperate, SqlDatabaseOperate, sql_models::fetcher::datasource_config::models::model_datasource_config::DatasourceBasicInfo};
use fetcher::{datasource_combination::ToDatasourceCombination, ToFetcher, datasource_config::{ToDatasource, OperateError as DatasourceOperateError}};
use futures::future;
use tokio::task::{JoinHandle, self};

use crate::{error::LogicResult, view::{CookieListReq, CookieTempListResp, CookieListResp, SingleCookie, DefaultCookie}};
...
impl CeobeCookieLogic {
    pub async fn cookie_list(
        ...
        let cookie_list = task::spawn(async move {
            mongo_copy.ceobe().cookie().analyze().get_data_by_paginate(
        ...
        let next_cookie_id = task::spawn(async move {
            mongo.ceobe().cookie().analyze().get_next_page_cookie_id(
                cookie_info.cookie_id,
                datasource_vec_copy,
                10,
            ).await
        ...
        let datasource_info:JoinHandle<Result<_, DatasourceOperateError>> = task::spawn(async move {
            let basic_info = db_copy.fetcher().datasource().find_basic_info_by_ids(datasource_vec).await?;
            Ok(basic_info.into_iter().map(|info| (info.id, info)).collect::<HashMap::<i32, DatasourceBasicInfo>>())
        });
        ...
        for cookie in cookie_list {
            cookie_list_resq.push(
                SingleCookie::builder()
                    .datasource(datasource_info[&cookie.source_config_id].nickname.clone())
